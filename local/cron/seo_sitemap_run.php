...
//0 0 * * * php -f /home/bitrix/ext_www/belwooddoorskz.razrabotka.by/bitrix/php_interface/cron/seo_sitemap_run.php action=sitemap_run ID=1 lang=ru > /home/bitrix/ext_www/belwooddoorskz.razrabotka.by/bitwqrix/php_interface/cron/cron.log
// 0 0 * * * php -f /home/bitrix/www/local/cron/seo_sitemap_run.php action=sitemap_run ID=1 lang=ru > /home/bitrix/www/local/cron/cron.log
$_SERVER['DOCUMENT_ROOT'] = "/home/bitrix/ext_www/belwooddoors.ru";
//$_SERVER['DOCUMENT_ROOT'] = "/home/bitrix/ext_www/belwooddoorsru.razrabotka.by";
require_once($_SERVER['DOCUMENT_ROOT']."/bitrix/modules/main/include/prolog_before.php");

use Ml\Main\Map\MapTable;

if (!MapTable::getEntity()->getConnection()->isTableExists(MapTable::getTableName())) {
    MapTable::getEntity()->createDbTable();
}
...
if($_REQUEST['action'] == 'sitemap_run')
{
    ...
    $connection->truncateTable(MapTable::getTableName());
        ...
        elseif($v < $arValueSteps['files']) {
            ...
            while(!$bFinished && microtime(true) <= $ts_finish) {
                ...
                if($arRes = $dbRes->Fetch()) {
                    ...
                    if($arRes["ITEM_PATH"]){
                        $sSectionName = '';
                        if(file_exists($_SERVER["DOCUMENT_ROOT"].$arRes["ITEM_PATH"]."/.section.php")){
                            include ($_SERVER["DOCUMENT_ROOT"].$arRes["ITEM_PATH"]."/.section.php");
                        }

                        if(mb_strlen($arRes["ITEM_PATH"])>1){
                            $arRes["ITEM_PATH"].="/";
                        }
                        ...
                        MapTable::add([
                            ...
                            "NAME"=>$sSectionName?:'',
                            "URL"=>$arRes["ITEM_PATH"]?:'',
                            "DEPTH_LEVEL"=>$arRes["ITEM_PATH"]?count(explode('/',$arRes["ITEM_PATH"]))-2:'1',
                            "STATIC"=>true,
                        ]);
                    ...
                    if($dbIblockResult == null) {
                        if(isset($NS['CURRENT_SECTION'])) {
                            $dbIblockResult = CIBlockElement::GetList(
                                ...
                                array('ID', 'TIMESTAMP_X', 'DETAIL_PAGE_URL','NAME')
                            );
                        }
                        else {
                            $NS['LAST_ELEMENT_ID'] = 0;
                            $dbIblockResult        = CIBlockSection::GetList(
                                ...
                                false,
                                array(
                                    'ID', 'TIMESTAMP_X', 'SECTION_PAGE_URL', 'LEFT_MARGIN', 'IBLOCK_SECTION_ID','NAME',
                                ),
                                array('nTopCount' => 100)
                            );
                        }
                    ...
                    if(isset($NS['CURRENT_SECTION'])) {
                        ...
                        if($arElement) {
                            ...
                            MapTable::add([
                                ...
                                "NAME"=>$arElement["NAME"]?:'',
                                "URL"=>$url?:'',
                                "DEPTH_LEVEL"=>$url?count(explode('/',$url))-2:'1',
                                "STATIC"=>false,
                            ]);
