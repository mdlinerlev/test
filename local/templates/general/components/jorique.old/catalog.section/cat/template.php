	...
	if (!empty($arResult['SKU_PROPS']))
	{
		foreach ($arResult['SKU_PROPS'] as $arProp)
		{
			...
			elseif ('PICT' == $arProp['SHOW_MODE'])
			{
				...
				{
					$value['NAME'] = htmlspecialcharsbx($value['NAME']);
					//$skuTemplate[$propId]['ITEMS'][$value['ID']] = '<a data-treevalue="'.$propId.'_'.$value['ID'].'" data-onevalue="'.$value['ID'].'"'.'class="catalog-item-aside__color" style="background-image:url(\''.$value['PICT']['SRC'].'\');" title="'.$value['NAME'].'"></a>';
				$skuTemplate[$propId]['ITEMS'][$value['ID']] = '<div class="tooltip1"><a data-treevalue="'.$propId.'_'.$value['ID'].
                    '" data-onevalue="'.$value['ID'].'"'.
					'class="catalog-item-aside__color" title=""><img class="imgcolor" src="'.$value['PICT']['SRC'].'" /><span class="tooltiptext">'.$value['NAME'].'</span></a></div>';
				}
				unset($value);
			}
		}
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?><div class="catalog-item" id="<? echo $strMainID; ?>" data-product-id="<?= $arItem['ID'] ?>">
...
?>
		...
		<div>
			...
			if($arItem['PREVIEW_PICTURE']['ID']) {
				$arItem['PREVIEW_PICTURE']['SRC'] = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array(
					'width' => 170,
					'height' => 280
			...
			?>
                    <a id="<? echo $arItemIDs['PICT']; ?>" href="/catalog/<?echo $ar_res["CODE"].'/'.$arItem["CODE"]; ?>/<?= $hash ?>" style="" class="catalog-item__image-link">
	...
	?><div class="catalog-item__title-container"><a class="catalog-item__title" href="/catalog/<?echo $ar_res["CODE"].'/'.$arItem["CODE"]; ?>/<?= $hash ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a></div>
	...
	if (!empty($minPrice))
	{
		...
		{?>
            <?php if ($arItem['PROPERTIES']['BOX_SQUARE']['VALUE']) { ?>
				<div class="catalog-item-price__discount" id="<? echo $arItemIDs['PRICE']; ?>"><?echo SaleFormatCurrency($minPrice['DISCOUNT_VALUE'] / $arItem['PROPERTIES']['BOX_SQUARE']['VALUE'], $minPrice['CURRENCY']);?>/м<sup>2</sup></div>
			<?php } else { ?>
		...
		if ('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE'])
		{
           ...
           <?if (!$arItem["OFFERS"]){?> <span class="catalog-item-price__number"><? echo $minPrice['PRINT_VALUE']; ?>
           </span><?}?>
           
           <span class="catalog-item-price__badge"><?=$minPrice['DISCOUNT_DIFF_PERCENT'];?> %</span>
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		if ($arItem['CAN_BUY'])
		{
            ...
            <a id="<?// echo $arItemIDs['BUY_LINK']; ?>" class="catalog-item__button catalog-item__button--more button" href="/catalog/<?echo $ar_res["CODE"].'/'.$arItem["CODE"]; ?>/">Подробнее<?
			...
			*/?></a>
			...
			{
				?>
                              <!--noindex-->  <div id="<?=$arItemIDs["BASKET_ACTIONS"]?>" class="catalog-item__compare">
                                    <a rel="nofollow" id="<? echo $arItemIDs['COMPARE_LINK']; ?>"  href="<?= str_replace('/catalog/compare/', '', $arItem["COMPARE_URL"])?>"><span class="catalog-item-compare__text catalog-item-compare__text--default"><? echo $compareBtnMessage; ?></span></a>
				</div><!--/noindex--><?
			}
		}
		else
		{
			...
			?>
				...
				?>
                                <!--noindex-->
				<a rel="nofollow" id="<? echo $arItemIDs['SUBSCRIBE_LINK']; ?>"  href="<?= str_replace('/catalog/compare/', '', $arItem["COMPARE_URL"])?>"><?
			...
			if ($arItem['OFFERS_PROPS_DISPLAY'])
				{
					foreach ($arItem['JS_OFFERS'] as $keyOffer => $arJSOffer)
					{
						...
						if (!empty($arJSOffer['DISPLAY_PROPERTIES']))
						{
							...
							{
                                                            if ($arOneProp['NAME'] == 'Старая цена' && $arOneProp['VALUE'] > $arItem["PROPERTIES"]["MINIMUM_PRICE"]["VALUE"]+1 ){
								$strProps .= '<div class="catalog-item__badge catalog-item__badge--discount">%
                      </div>';
                                                                //echo '<div class="catalog-item-price__base"><span class="catalog-item-price__number">'.($arOneProp['VALUE']).' <?=MAIN_CURRENCY_TEXT?></span></div>';
                                                                ?><?
                                                                 }
/*
if($arOneProp['NAME'] == 'Складская программа' && $arOneProp['VALUE'] == 'Y' ){
$strProps .= '<div class="catalog-item__badge catalog-item__badge--stores">Складская программа</div>';
}*/
//добавить стикер "складская программа" ко всем ТП с количеством >0
                                //Проходим по всем торговым предложениям, если гдето количество товара боле 0 выводим стикер
                                if($arOneProp['NAME'] == 'Складская программа' && $arOneProp['VALUE'] == 'Y' ){
									// $strProps .= '<div class="catalog-item__badge catalog-item__badge--stores">Складская программа</div>';
									 foreach ($arItem['OFFERS'] as $offer_item){
                                        if($offer_item['CATALOG_QUANTITY'] > 0){
                                            $strProps .= '<div class="catalog-item__badge catalog-item__badge--stores">'.$arOneProp['NAME'].'</div>';
                                            break;
                                        }
                                    }
...
<div class="catalog-item__bottom">
		...
		<?/*<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="catalog-item__compare" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">*/?>
                    <a id="<?// echo $arItemIDs['BUY_LINK']; ?>" class="catalog-item__button catalog-item__button--more button" href="/catalog/<?echo $ar_res["CODE"].'/'.$arItem["CODE"]; ?>/"><?
			/*if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY')
			{
				echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
			}
    ...
    <!--noindex-->
                    <div id="<?=$arItemIDs["BASKET_ACTIONS"]?>" class="catalog-item__compare">
                        <a rel="nofollow" id="<? echo $arItemIDs['COMPARE_LINK']; ?>"  href="javascript:void;<?//= str_replace('/catalog/compare/', '', $arItem["COMPARE_URL"])?>"><span class="compare_icon catalog-item-compare__text--default"></span><span class="catalog-item-compare__text catalog-item-compare__text--default"><? echo $compareBtnMessage; ?></span></a>
</div>   
 <?}else{?>
                       <div id="<?=$arItemIDs["BASKET_ACTIONS"]?>" class="catalog-item__compare active">
<a rel="nofollow" id="<? echo $arItemIDs['COMPARE_LINK']; ?>"  href="/catalog/compare/"><span class="catalog-item-compare__text catalog-item-compare__text--active"><?// echo $compareBtnMessage; ?>В сравнении</span></a>
                       </div><!--/noindex-->
<?}?>
<?
	}
...
<?
				...
				?>
                           ...
                           <? }elseif(($arOneProp['ID'] == 100) && ($arOneProp['VALUE'] > 0)){?>
                               <div class="catalog-item__badge catalog-item__badge--discount">%
                      </div>
...
.catalog-item-aside__color img{
		  width: 23px;
		height: 23px;		
}
	.imgcolor {width: 23px;    top: 0px;
    position: absolute;}
</style>