...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
 ...
 foreach ($arItem["OFFERS"] as $off => $offfer){
     
   //  __($arItem["OFFERS"]);
     ?>  
		<? if ($offfer["PROPERTIES"]["OLD_PRICE"]["VALUE"] > $minPrice['VALUE']){?>
...
?>
		...
		<div>
                    <a id="<? echo $arItemIDs['PICT']; ?>" href="/catalog/<?echo $ar_res["CODE"].'/'.$arItem["CODE"]; ?>/<?= $arItem["OFFERS"] ? '#offer'.$arItem['OFFERS'][0]['ID'] : '' ?>" style="" class="catalog-item__image-link">
	...
	if (!empty($minPrice))
	{
		...
		<?}
           ...
           <span class="catalog-item-price__number"><?=$arItem["OFFERS"][$off]["PROPERTIES"]["OLD_PRICE"]["~VALUE"]?> руб.</span>
            <span class="catalog-item-price__badge"><?=number_format((($arItem["OFFERS"][$off]["PROPERTIES"]["OLD_PRICE"]["~VALUE"]-$minPrice['PRINT_VALUE'])/($arItem["OFFERS"][$off]["PROPERTIES"]["OLD_PRICE"]["~VALUE"]/100)), 0);?>  %</span>
