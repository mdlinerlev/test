...
if(CModule::IncludeModule("sphinx") && !empty($query)){
	$sphinxSearch = new CSphinxSearch(1);
	$sphinxSearch->SetLimits(0, 50);
    ...
    $result = $sphinxSearch->DoSearch($query, $filter);
...
if (CModule::IncludeModule("catalog") and CModule::IncludeModule("iblock")) {
    ...
    $arFilter = Array(
        ...
        "%NAME" => htmlspecialcharsbx($_REQUEST["q"]),
    );
...
if (!empty($arElements) && is_array($arElements)):
    ...
    $APPLICATION->SetPageProperty('search-found', 'catalog-search-nothing-found');
    if(!empty($query)):
        ?>
        <div class="search-nothing-found">
            <?ShowNote(GetMessage("CT_BCSE_NOT_FOUND", array("#query#" => $APPLICATION->GetProperty('search-query'))));?>
            <br>
