...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?><div class="catalog-item"id="<? echo $strMainID; ?>">
		...
		<div>
                    <a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" style="background-size: cover;" class="catalog-item__image-link">
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		<div class="bx_catalog_item_controls_blockone"><div style="display: inline-block;position: relative;">
			...
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="catalog-item__button catalog-item__button--more button" href="<?=$arItem['DETAIL_PAGE_URL']?>" rel="nofollow">Подробнее<?
			...
			*/?></a>
			...
			if ($arParams['DISPLAY_COMPARE'])
			{
				?>
					...
					<a id="<? echo $arItemIDs['COMPARE_LINK'].'&ajax_action=Y'; ?>" class="catalog-item-compare__text catalog-item-compare__text--default" href="<?=$arItem["COMPARE_URL"].'&ajax_action=Y'?>"><? echo $compareBtnMessage; ?></a>
				</div><?
			}
		}
		else
		{
			...
			?>
				...
				if ($arParams['DISPLAY_COMPARE'])
				{
					?><a id="<? echo $arItemIDs['COMPARE_LINK']; ?>" class="catalog-item-compare__text catalog-item-compare__text--default" href="<?=$arItem[""]?>"><? echo $compareBtnMessage; ?></a><?
				}
				if ($showSubscribeBtn)
				{
				?>
				<a id="<? echo $arItemIDs['SUBSCRIBE_LINK']; ?>" class="catalog-item-compare__text catalog-item-compare__text--default" href="javascript:void(0)"><?
			...
			foreach ($arItem['DISPLAY_PROPERTIES'] as $arOneProp)
			{
				?>
                            ...
                            <?}elseif(($arOneProp['ID'] == 6) && ($arOneProp['VALUE'])){?>
                                <div class="catalog-item__badge catalog-item__badge--new">Новое
                      </div>
...
<?if(in_array($arItem["IBLOCK_SECTION_ID"], array(35,36))){?>
		...
		<?/*<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="catalog-item__compare" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">*/?>
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="catalog-item__button catalog-item__button--more button" href="<?=$arItem["DETAIL_PAGE_URL"]?>" rel="nofollow"><?
			/*if ($arParams['ADD_TO_BASKET_ACTION'] == 'BUY')
			{
				echo ('' != $arParams['MESS_BTN_BUY'] ? $arParams['MESS_BTN_BUY'] : GetMessage('CT_BCS_TPL_MESS_BTN_BUY'));
			}
	...
	?>
<div id="<?=$arItemIDs["BASKET_ACTIONS"]?>" class="catalog-item__compare">
	<a id="<? echo $arItemIDs['COMPARE_LINK']; ?>" class="catalog-item-compare__text catalog-item-compare__text--default" href="javascript:void(0)"><? echo $compareBtnMessage; ?></a>
</div><?
	}
	?>
...
<?}
		...
		else
		{
			?>
		<div class="catalog-item__bottom">
			<a class="catalog-item__button catalog-item__button--more button" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"><?
...
<script>
$('.catalog-item-compare__text').on('click', function() {
  if( $(this).is('.active') ) {
   var that = this,
    href = $(this).attr('href'); 
   $.post(href, {}, function() {
