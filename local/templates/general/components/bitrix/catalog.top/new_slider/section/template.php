...
<section class="items-slider items-slider--new">
    <div class="content-container">
	    <div class="items-slider__title-container">
		    <div class="items-slider__title">Новинки</div>
		    <a href="<?=SITE_DIR?>new/" class="items-slider__button items-slider__button--all-button">
        ...
        foreach ($arResult['ITEMS'] as $key => $arItem) {
 		...
 		$imgTitle = (
   		...
   		$imgAlt = (
                isset($arItem['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_ALT']) && $arItem['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_ALT'] != '' ? $arItem['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_ALT'] : $arItem['NAME']
                );
            ...
            <div class="catalog-item <?=($arItem['PROPERTIES']['CONFIGURATION']['VALUE'] == $confDoor && $arItem['PROPERTIES']['DOUBLE_IMAGE']['VALUE'] == 1? 'double_image' : '');?>">
                <div class="catalog-item__inner" id="<? echo $strMainID; ?>">
                    <div class="catalog-item__image-container">
                        <div>
                            <?$file = CFile::ResizeImageGet($arItem['OFFERS'][0]['PREVIEW_PICTURE'], array('width'=>135,'height' => 280), BX_RESIZE_IMAGE_PROPORTIONAL, true, $arWaterMark);?>
                            <a id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL'].'#offer'.$arItem['OFFERS'][0]['ID']; ?>" class="catalog-item__image-link">
                                <?if($arItem['PROPERTIES']['CONFIGURATION']['VALUE'] == $confDoor && $arItem['PROPERTIES']['DOUBLE_IMAGE']['VALUE'] == 1) {?>
                                    <img src="<?= SITE_TEMPLATE_PATH ?>/preload.svg"
                                         data-src="<? echo $arItem['OFFERS'][0]['PREVIEW_PICTURE_ORIG']['SRC']; ?>"
    ...
    /*if ($arItem['LABEL']) {
        ?>
                        ...
                        if ($arItem['SECOND_PICT']) {
                            ?>
                    <?$file_sec = CFile::ResizeImageGet($arItem['OFFERS'][0]['PREVIEW_PICTURE'], array('width'=>135,'height' => 280), BX_RESIZE_IMAGE_PROPORTIONAL, true, $arWaterMark);?>
                        <a id="<? echo $arItemIDs['SECOND_PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL'].'#offer'.$arItem['OFFERS'][0]['ID']; ?>" class="bx_catalog_item_images_double" style="background-image: url('<?
                            echo (
                            !empty($arItem['PREVIEW_PICTURE_SECOND']) ? $file_sec['src'] : $file['src']
                            );
                            ?>')" title="<? echo $imgTitle; ?>">
                        <?/*
                        if ('Y' == $arParams['SHOW_DISCOUNT_PERCENT']) {
                    ...
                    <div class="catalog-item__price">
                        ...
                        if (!empty($minPrice)) {
                            ...
                            if ('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE']) {
                                ?><div class="catalog-item-price__base"> <span class="catalog-item-price__number"><? echo $minPrice['PRINT_VALUE']; ?></span>
                    ...
                    <div class="catalog-item__buttons">
	                    <a class="button button--w-icon" href="<? echo $arItem['DETAIL_PAGE_URL'].'#offer'.$arItem['OFFERS'][0]['ID']; ?>">
	    ...
	    <div class="items-slider__footer mobile_only">
		    <a href="/catalog/" class="button button--secondary">Открыть каталог дверей</a>
	    </div>
    </div>
</section>
