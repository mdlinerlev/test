...
foreach ($items as $arItem) {
    ...
    if ($arPrice = CCatalogProduct::GetOptimalPrice(
        ...
        if($arItem['procent'] > 0) {
            $arItem['price'] = number_format(($arPrice['DISCOUNT_PRICE'] + $arPrice['DISCOUNT_PRICE'] * $arItem['procent'] / 100), 2, ",", " ");
            $arItem['price_sum_val'] = ($arPrice['DISCOUNT_PRICE'] * $arItem['procent'] / 100 + $arPrice['DISCOUNT_PRICE']) * $arItem['count'];
            $arItem['price_sum'] = number_format(($arPrice['DISCOUNT_PRICE'] + $arPrice['DISCOUNT_PRICE'] * $arItem['procent'] / 100) * $arItem['count'], 2, ",", " ");

            $totalPrice += ($arPrice['DISCOUNT_PRICE'] + $arPrice['DISCOUNT_PRICE'] * $arItem['procent'] / 100) * $arItem['count'];
        } else {
    ...
    $itemsResult[$arItem['id']]['temp_name'] = '';
    $itemsResult[$arItem['id']] = $arItem;
    if($arItem['height'] && $arItem['width'])
        $itemsResult[$arItem['id']]['temp_name'].= ' ( Высота: '.$arItem['height']. ', Ширина: '. $arItem['width'].' )';

    if($arItem['color'])
        $itemsResult[$arItem['id']]['temp_name'].= ' ( Цвет RAL: '.$arItem['color'].' )';
}
...
if (!empty($itemsId)) {
    ...
    while ($arItem = $iterator->fetch()) {
        ...
        $itemsResult[$arItem['ID']]['name'].= $itemsResult[$arItem['ID']]['temp_name'];
