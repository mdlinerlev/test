...
//                if (BX('old_allSum_FORMATED'))
//			BX('old_allSum_FORMATED').innerHTML = number_format(res['BASKET_DATA']['allSum']*10000, 0, '.', ' ') + ' руб.';
	...
	if (bIsCorrectQuantityForRatio)
	{
		...
		if (autoCalculate)
		{
			basketPoolQuantity.changeQuantity(basketId);
                ...
                for (basketId in basketPoolQuantity.currentQuantity){
                    total_quant += parseInt(basketPoolQuantity.currentQuantity[basketId]);
                   
                }
                var total_quantity = document.getElementById('all_quantity'); 
total_quantity.innerHTML = total_quant;
	}
	else
	{
...
function get_total_quantity(){
    var basketId;
    var all_quantity;
    BX("QUANTITY_INPUT_" + basketId).each(function(){
    all_quantity = BX("QUANTITY_INPUT_" + basketId).val;
    });
    $('#all_quantity').innerHTML = all_quantity;
    console.log('1');
};