;(function () {
    ...
    BX.Sale.BasketComponent = {
        ...
        bindTotalEvents: function (node) {
            ...
            BX.bind(this.getEntity(node, 'stock-set'), 'click', BX.proxy(this.setStockPercent, this));
            BX.bind(this.getEntity(node, 'basket-total-delete'), 'click', BX.proxy(this.ClearAll, this));
            BX.bind(this.getEntity(node, 'basket-total-kpcreate'), 'click', BX.proxy(this.KpCreateModal, this));
        ...
        KpCreateModal: function (event) {
            var target = BX.getEventTarget(event);
            var url = $(target).attr('data-href');
            var mainClass = $(target).attr('data-class');
        ...
        setStockPercent: function (event) {
            var entity = this.getEntity(document, 'stock-value');
                ...
                success: (result) => {
                    if(!result.success){
                        ...
                        var messageText =
                            ...
                            '</div>' +
                            '<div class="popup-b2b__form">' + result.errorMsg + '</div>' +
                            '</div>' +
                            '</div>';
