...
<div class="container-gray__header">
	<div class="container-gray__title">Отзывы наших клиентов</div>
    <a class="container-gray__all" href="<?=SITE_DIR?>about/comments/" class="title-through">все отзывы</a>
</div>
<div class="index-feedback__slider">
...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	<div class="feedback__inner" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
        <div class="feedback__date"><?=$arItem["DISPLAY_ACTIVE_FROM"]?>
        </div>

        <div class="feedback__text"><?=$arItem["DETAIL_TEXT"]?></div>
        <div class="feedback__title"><?=$arItem["NAME"]?></div>

        <? $rating = strlen($arItem["PROPERTIES"]["RATING"]["VALUE"]) > 0 ? str_replace(',', '.', $arItem["PROPERTIES"]["RATING"]["VALUE"])
	        : str_replace(',', '.', $arItem["PROPERTIES"]["RATING"]["DEFAULT_VALUE"])?>
...
<div class="feedback__buttons">
              <a href="javascript:void(0)" class="feedback__button feedback__button--more button"><span>Показать еще</span>
              </a>
              <a class="feedback__button feedback__button--show-form button"><span>Оставить отзыв</span>
              </a>
            </div>*/?>
