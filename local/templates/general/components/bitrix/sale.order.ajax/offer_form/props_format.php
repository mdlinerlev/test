...
if (!function_exists("showFilePropertyField")) {
    ...
    function showFilePropertyField($name, $property_fields, $values, $max_file_size_show = 50000) {
        ...
        if ($property_fields["MULTIPLE"] == "N") {
            $res = "<div class='order__form-group order__form-group--files'>
                <div class='order__files-controls'>
                      <div class='order__files-title'>Укажите реквизиты
                      </div>
                      <div class='order__files-or'>или
                      </div>
                      <input type=\"file\" size=\"" . $max_file_size_show . "\" value=\"" . $property_fields["VALUE"] . "\" name=\"" . $name . "[0]\" id=\"" . $name . "[0]\" class='order__file-input'>"
                    . "<a class='order__button order__button--file button'><span>Добавьте файл с реквизитами</span>
                      </a>"
                    . "</div></div>";
        } else {
            $res = '
    ...
    function PrintPropsForm($arSource = array(), $locationTemplate = ".default") {
        if (!empty($arSource)) {
            ...
            foreach ($arSource as $arProperties) {
                ?>
                <div class="order__form-group" data-property-id-row="<?= intval(intval($arProperties["ID"])) ?>">

                    <label for="<?= $arProperties["FIELD_NAME"] ?>" class="order__label">
                <?= $arProperties["NAME"] ?>
                ...
                <? endif ?>
                    ...
                    elseif ($arProperties["TYPE"] == "TEXT") {
                        ...
                        <input class="order__input" placeholder="<?= $arProperties["VALUE"] ?>" type="text" maxlength="250" size="<?= $arProperties["SIZE1"] ?>" value="<?= $arProperties["VALUE"] ?>" name="<?= $arProperties["FIELD_NAME"] ?>" id="<?= $arProperties["FIELD_NAME"] ?>" />
                            <? if (strlen(trim($arProperties["DESCRIPTION"])) > 0): ?>
                            <div class="order__message-container">
                                <div>
                                    <div class="order__message order__message--comment"><?= $arProperties["DESCRIPTION"] ?></div>
                                </div>
                            </div>
                        <? endif ?>
                ...
                elseif ($arProperties["TYPE"] == "MULTISELECT") {
                    ...
                    elseif ($arProperties["TYPE"] == "TEXTAREA") {
                        ...
                        <textarea class="order__textarea order__textarea--address" rows="<?= $rows ?>" cols="<?= $arProperties["SIZE1"] ?>" name="<?= $arProperties["FIELD_NAME"] ?>" id="<?= $arProperties["FIELD_NAME"] ?>"><?= $arProperties["VALUE"] ?></textarea>
                            <? if (strlen(trim($arProperties["DESCRIPTION"])) > 0): ?>
                            <div class="order__message-container order__message-container--textarea">
                                <div>
                                    <div class="order__message order__message--comment"><?= $arProperties["DESCRIPTION"] ?></div>
                                </div>
                            </div>
