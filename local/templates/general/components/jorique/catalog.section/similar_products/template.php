    ...
    foreach ($arResult['SKU_PROPS'] as $arProp) {
        ...
        if ('TEXT' == $arProp['SHOW_MODE']) {
            ...
            foreach ($arProp['VALUES'] as $value) {
                ...
                $skuTemplate[$propId]['ITEMS'][$value['ID']] = '<a data-offer-id="#OFFER_ID#" style="visibility: visible" data-perent="#PRODUCT_ID#" data-treevalue="' . $propId . '_' . $value['ID'] .
            ...
            foreach ($arProp['VALUES'] as $value) {
                ...
                $skuTemplate[$propId]['ITEMS'][$value['ID']] = '<div class="tooltip1"><a data-offer-id="#OFFER_ID#" style="visibility: visible" data-perent="#PRODUCT_ID#" data-treevalue="' . $propId . '_' . $value['ID'] .
                    '" data-onevalue="' . $value['ID'] . '"' .
                    'class="catalog-item-aside__color" title=""><img class="imgcolor" style="width: 23px; height: 23px;" src="' . SITE_TEMPLATE_PATH . '/preload.svg" data-src="' . $value['PICT']['SRC'] . '" alt="" /><span class="tooltiptext">' . $value['NAME'] . '</span></a></div>';
            }
            unset($value);
        }
    }
...
<div class="catalog__list <?= $ibFirstSectionIdCode == 'napolnye_pokrytiya' ? ' catalog--floor' : ''?>" style="height: 970px;">
    <div class="catalog__list_inner">
        ...
        <div class="catalog-item <?=($arItem['PROPERTIES']['CONFIGURATION']['VALUE'] == $confDoor && $arItem['PROPERTIES']['DOUBLE_IMAGE']['VALUE'] == 1? 'double_image' : '');?> <?= is_int($n / 4) ? "right-tut": ""?> similar" id="<? echo $strMainID; ?>" data-product-id="<?= $arItem['ID'] ?>" style="">
            <?$n++;?>
            <div class="catalog-item__inner <?= empty($shadow) ? ' catalog-item__bg_small' : ''?>">
                ...
                <div class="catalog-item__top">
                            ...
                            <a href="<?=$linkProduct.$hash ?>"
                               class="catalog-item__image-link">
                                <?if($arItem['PROPERTIES']['CONFIGURATION']['VALUE'] == $confDoor && $arItem['PROPERTIES']['DOUBLE_IMAGE']['VALUE'] == 1) {?>
                                            <img id="<? echo $arItemIDs['PICT']; ?>_1"  class="catalog-item__image <?=$dveri ? 'dveri' : ''?>"
                                        <?//= $shadow ? ' style="opacity: 0;" ' : ''?>
                                         data-src="<? echo $arItem['PREVIEW_PICTURE_ORIG']['SRC']; ?>"
                                         src="<?= SITE_TEMPLATE_PATH ?>/preload.svg" alt="<?= $imgAlt?>" title="<?= $imgTitle?>">
                                <?}?>
                                            <img id="<? echo $arItemIDs['PICT']; ?>"  class="catalog-item__image <?=$dveri ? 'dveri' : ''?>"
                                    <?//= $shadow ? ' style="opacity: 0;" ' : ''?>
                    ...
                    <div class="catalog-item__title-container">
                        <a class="catalog-item__title" href="<?=$linkProduct. $hash ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a>
                    </div>
                        ...
                        if (!empty($minPrice)) {
                            if ('N' == $arParams['PRODUCT_DISPLAY_MODE'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
                                ...
                                <div class="catalog-item-price__discount" id="<? echo $arItemIDs['PRICE']; ?>"><?
                                    echo current($arItem['OFFERS'])["CATALOG_PRICE_1"]; ?></div>
                                <?
                            }
                            if ('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE']) {
                                ...
                                <?
                                if (!$arItem["OFFERS"]) {
                                    ?> <span class="catalog-item-price__number"><? echo $minPrice['PRINT_VALUE']; ?>
                                    </span><?
                                } ?>

                                <span class="catalog-item-price__badge"><?= $minPrice['DISCOUNT_DIFF_PERCENT']; ?>
                                    <?= GetMessage('CT_BCS_DISCOUNT_DIFF_PERCENT')?></span>
                ...
                if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])) {
                    ...
                    <div class="catalog-item__bottom"><?
                        if ($arItem['CAN_BUY']) {
                            ...
                            <div class="one-click">
                                <script data-b24-form="click/20/15boem" data-skip-moving="true"> (function(w,d,u){ var s=d.createElement('script');s.async=true;s.src=u+'?'+(Date.now()/180000|0); var h=d.getElementsByTagName('script')[0];h.parentNode.insertBefore(s,h); })(window,document,'https://bitrix.belwood.ru/upload/crm/form/loader_20_15boem.js'); </script>
                                <div data-product_id="<?=$arItem['ID']?>" class="one-click-buy button">Заказать в 1 клик</div> <!--data-popup="popup-oneclickbuy"-->
                            </div>

                            <a class="catalog-item__button catalog-item__button--more button button--secondary" href="<?= $linkProduct?>"><?= GetMessage("CT_BCS_TPL_MESS_BTN_DETAIL")?></a>
                                ...
                                */?>
                                <div id="<?/*= $arItemIDs["BASKET_ACTIONS"] */?>" class="catalog-item__compare">
                                    <a rel="nofollow" id="<?/* echo $arItemIDs['COMPARE_LINK']; */?>"
                                       ...
                                       */?>"><span class="compare_icon catalog-item-compare__text--default"></span><span
                                                class="catalog-item-compare__text catalog-item-compare__text--default"><?/* echo $compareBtnMessage; */?></span></a>
                                </div>
                                <?/*
                            } else {
                                */?>
                                <div id="<?/*= $arItemIDs["BASKET_ACTIONS"] */?>" class="catalog-item__compare active">
                                    <a rel="nofollow" id="<?/* echo $arItemIDs['COMPARE_LINK']; */?>"
                                       href="/catalog/compare/"><span
                                                class="catalog-item-compare__text catalog-item-compare__text--active"><?/* // echo $compareBtnMessage;
                                            */?><?/*= GetMessage('CT_BCS_TPL_MESS_BTN_COMPARE_ADD')*/?></span></a>
                                </div>
                                --><?/*
                            } */?>
                            <?
                ...
                ?>
                    <div class="catalog-item__badge-container">
                        ...
                        <? if ($boolShowOfferProps) {
                            ?>
                            <span id="<? echo $arItemIDs['DISPLAY_PROP_DIV']; ?>" class="catalog-item__label_offer" style="display: none;"></span>
                            <?
                        }
                        ?>
                ...
                ?>
                    ...
                    foreach ($skuTemplate as $propId => $propTemplate) {
                        ...
                        foreach ($propTemplate['ITEMS'] as $value => $valueItem) {
                            ...
                            echo str_replace(array('#ITEM#_prop_', '#WIDTH#', '#OFFER_ID#', '#PRODUCT_ID#'), array($arItemIDs['PROP'], $itemWidth, $offer_id, $arItem["ID"]), $valueItem);
                        }
