...
<? function inducementWord($number, $wordArr)
{
    $cases = array(2, 0, 1, 1, 1, 2);
    return $wordArr[($number % 100 > 4 && $number % 100 < 20) ? 2 : $cases[min($number % 10, 5)]];
} ?>

<div id="skyweb24_coupon_type1"
    ...
    <div class="bg" style="background-image:url('<?= $arResult['IMG_1_SRC'] ?>')">
        <div>
            ...
            <span class="sale"><?= GetMessage("POPUPPRO_SALE") ?> <?= $arResult['PERCENT'] ?></span><span
                    class="avaliable"><?= GetMessage("POPUPPRO_AVALIABLE") ?> <?= $arResult['TIMING'] ?>
            ...
            <label class="<?= $arResult['EMAIL_SHOW'] ?> input">
                ...
                <input type="email" value="<?= $arResult['EMAIL'] ?>" name="EMAIL"
...
<script>
    ...
    if(BX)
    {
        BX.ready(function () {
            ...
            let popupContainer = BX("skyweb24_coupon_type1");
