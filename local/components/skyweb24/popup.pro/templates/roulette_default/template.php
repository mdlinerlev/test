...
<?php if (!is_numeric(strpos($APPLICATION->GetCurPage(), "admin"))): ?>
    <style>
        :root {
        <?php foreach ($arResult['theme_color']['COLORS'] as $key => $value): ?><?="--" . strtolower($key) . ":" . $value['VALUE'] . ";";?><?php endforeach; ?>
        }
    </style>
    <?php if (is_numeric(strpos($_SERVER['HTTP_USER_AGENT'], "Trident"))) : ?>
        <script>cssVars();</script>
    <?php endif; ?>
<?php endif; ?>

<?php if ($arResult['TIMER'] == 'Y') {
...
<div id="skyweb24_roulette"
     style="<?php if (!empty($arResult['GOOGLE_FONT'])) { ?>font-family:<?= $arResult['GOOGLE_FONT'] ?><?php } ?>">
    <?php if (!empty($arResult['GOOGLE_FONT'])) { ?>
        <link href="https://fonts.googleapis.com/css?family=<?= $arResult['GOOGLE_FONT'] ?>:400,700"
              rel="stylesheet"><?php } ?>
    <?php $deg = (!empty($arResult['ELEMENTS_COUNT'])) ? 100 / $arResult['ELEMENTS_COUNT'] : 0; ?>
...
<path style="fill:#FD003A;" d="M256,0C156.698,0,76,80.7,76,180c0,33.6,9.302,66.301,27.001,94.501l140.797,230.414
    ...
    <script>
        ...
        var dataset = [
            <?php foreach($arResult['ELEMENTS'] as $key => $element):?>
            {
                value: <?=$deg?>,
                color: '<?=$element['color']?>',
                text: '<?=$element['text']?>',
        ...
        async function roll_roulette_func(countSector) {
            ...
            let nodeInputPhone = BX('skyweb24_roulette').querySelector(`.sw24-roulette-input-phone`);
            let nodeInputEmail = BX('skyweb24_roulette').querySelector(`.sw24-roulette-input-email`);
        ...
        class RouletteGame {
            ...
            getAwardSectorWheel(numberSector) {
                var text = document.querySelector('#skyweb24_roulette div.rotate_block>.text>div:nth-child(' + numberSector + ')').innerHTML;
                var rule = document.querySelector('#skyweb24_roulette div.rotate_block>.text>div:nth-child(' + numberSector + ')').dataset.rule;
                var not = document.querySelectorAll('#skyweb24_roulette div.rotate_block>.container>div:not(:nth-child(' + numberSector + '))');
            ...
            renderAward(award) {
                if (this.isAwardWin(award.rule)) {
                    BX('skyweb24_roulette').querySelector(`.result_roll`).style.display = "block";
                } else {
                    BX('skyweb24_roulette').querySelector(`.result_roll_nothing`).style.display = "block";
                }
            }

            renderErrorMessage(message) {
                BX('skyweb24_roulette').querySelector(`.sw24-roulette-error-result`).style.display = "block";
                BX('skyweb24_roulette').querySelector(`.sw24-roulette-error-result`).innerHTML = message;
            }

            removeErrorMessage() {
                BX('skyweb24_roulette').querySelector(`.sw24-roulette-error-result`).innerHTML = "";
                BX('skyweb24_roulette').querySelector(`.sw24-roulette-error-result`).style.display = "none";
            }

            removeNodeButtonSend() {
                BX('skyweb24_roulette').querySelector('.btn-action').remove()
        ...
        <div>
            <?php if ($arResult['PHONE_FIELD_ACTIVE'] == "Y"): ?>
                <label class="email input">
                    <input placeholder="<?= $arResult['PHONE_PLACEHOLDER'] ?>" type="number" name="phone"
                           class="sw24-roulette-input-phone">
                </label>
            <?php endif; ?>
            <?php if ($arResult['EMAIL_SHOW'] == "Y"): ?>
                <label class="email input">
                    <input placeholder="<?= $arResult['EMAIL_PLACEHOLDER'] ?>" type="email" name="email"
                           class="sw24-roulette-input-email">
                    <p class="result_roll_fail" style="display:none;">
                        <?= GetMessage('skyweb24.roulette_wrong') ?>
                    </p>
                    <p class="result_roll_not_unique" style="display:none;">
                        <?= $arResult['EMAIL_NOT_NEW_TEXT'] ?>
                    </p>
                </label>
            <?php endif; ?>
        </div>
    ...
    <?php if (($arResult['CLOSE_TEXTBOX'] == 'Y') && (!empty($arResult['CLOSE_TEXTAREA']))) { ?>
        <div align="center">
            <a href="javascript:void(0);" class="sw24TextCloseButton">
                <?= $arResult['CLOSE_TEXTAREA'] ?>
            </a>
        </div>
    <?php } ?>
</div>
