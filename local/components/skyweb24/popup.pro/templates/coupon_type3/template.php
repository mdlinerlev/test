...
<div id="skyweb24_coupon_type3" style="<?if(!empty($arResult['GOOGLE_FONT'])){?>font-family:<?=$arResult['GOOGLE_FONT']?><?}?>;">
<?if(!empty($arResult['GOOGLE_FONT'])){?><link href="https://fonts.googleapis.com/css?family=<?=$arResult['GOOGLE_FONT']?>:400,700" rel="stylesheet"><?}?>
    ...
    <div class="container">
        <div class="fa fa-gift fa-5x" aria-hidden="true"></div>
        <div class="title"><?=$arResult['TITLE']?></div>
        <div class="subtitle"><?=$arResult['SUBTITLE']?></div>
        <div>
            <span class="sale"><?=GetMessage("POPUPPRO_SALE")?> <?=$arResult['PERCENT']?></span>
            <span class="avaliable"><?=GetMessage("POPUPPRO_AVALIABLE")?> <?=$arResult['TIMING']?> <?=GetMessage("POPUPPRO_DAYS_".inducementWord($arResult['TIMING'],array(1,2,5)))?></span>
        </div>
		<?
    ...
    <script>
        ...
        if(BX)
        {
            BX.ready(function () {
                ...
                let popupContainer = BX("skyweb24_coupon_type3");
