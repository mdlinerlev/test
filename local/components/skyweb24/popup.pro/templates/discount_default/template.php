...
<div class="skyweb24_discount" id="skyweb24_discount" style="<?if(!empty($arResult['GOOGLE_FONT'])){?>font-family:<?=$arResult['GOOGLE_FONT']?><?}?>">
    ...
    <div class="content">
        ...
        <fieldset>
			...
			<label class="submit"><input type="submit" onclick="" class="sw24-button-animate" value="<?=$arResult['BUTTON_TEXT']?>"></label>
...
<script>
	...
	function getCouponDisc<?=$arParams['ID_POPUP']?>(f){
        ...
        if(f.querySelector('input[name="CRM_SERVER_ID"]'))
        {
            var data = {
                CRM_SERVER_ID: f.querySelector('input[name="CRM_SERVER_ID"]').value,
                SOURCE_DESCRIPTION: f.querySelector('input[name="SOURCE_DESCRIPTION"]').value,
                TITLE: "<?=$arResult['SUBTITLE'];?>",
            }
        }
