...
if($this->StartResultCache(false, array('v10', $preFilter, ($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false))))
{
	...
	if(!empty($arResult["ITEMS"]))
	{
		if ($this->facet->isValid())
		{
            ...
            $arFilter123['!PROPERTY_CONFIGURATION'] = ['Распашная двойная','Купе','Купе двойное'];
            $customFilter = array_merge($arFilter123, $arResult["FACET_FILTER"]);
            $res2 = $this->facet->query($customFilter);
            while ($rowData = $res2->fetch()) {
                ...
                if (\Bitrix\Iblock\PropertyIndex\Storage::isPropertyId($facetId)) {
                    ...
                    $rowData['PID'] = $PID;
                    $tmpProperty2[$rowData['VALUE']] = $rowData;
                }
            }
			...
			while ($rowData = $res->fetch())
			{
				...
				if (\Bitrix\Iblock\PropertyIndex\Storage::isPropertyId($facetId))
				{
					...
					$rowData['PID'] = $PID;
                    if(isset($tmpProperty2[$rowData['VALUE']]) && $rowData['PID'] != CONFIGURATION) {
                        $rowData['ELEMENT_COUNT'] = $tmpProperty2[$rowData['VALUE']]['ELEMENT_COUNT'];
                    }
					$tmpProperty[] = $rowData;
