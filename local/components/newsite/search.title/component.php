...
$isSearchInstalled = CModule::IncludeModule("sphinx") && CModule::IncludeModule("iblock") && CModule::IncludeModule("search");
...
)
{
	...
	for($i = 0; $i < $arParams["NUM_CATEGORIES"]; $i++)
	{
		...
		if ($isSearchInstalled)
		{
			...
			$iblockId = $arParams["CATEGORY_".$i.'_'. ($arParams["CATEGORY_".$i][0])][0];
			$filter = array("FLOAT" => array("iblock_id" => $iblockId ? : 1));


			$sphinxSearch = new CSphinxSearch(1);
			$sphinxSearch->SetLimits(0, 100);

			$cl = $sphinxSearch->GetSphinxClientObject();

			$cl->ResetGroupBy();
			$cl->ResetFilters();

			$queryNew = mb_strtolower($arResult["query"]);
			$result = $sphinxSearch->DoSearch($queryNew, []);


			$elementIDFilter = array();
			foreach ($result["IBLOCK"] as $item) {
				$elementIDFilter[] = $item["id"];
			}

			$j = 0;
			$arElementSort = array();
			if(!empty($elementIDFilter)) {

				$dbl = CIBlockElement::GetList(array(), array(
					'IBLOCK_ID' => $iblockId,
					"ACTIVE_DATE" => "Y",
					"SECTION_ACTIVE" => 'Y',
					...
					'=ID' => array_values($elementIDFilter)
				), false, ["nPageSize" => 6], array("ID","IBLOCK_ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "PROPERTY_MINIMUM_PRICE"));
				while ($ar = $dbl->GetNext()) {

					if(!empty($ar['PREVIEW_PICTURE']))
						$ar["PREVIEW_PICTURE"] = CFile::ResizeImageGet($ar['PREVIEW_PICTURE'], array('width' => $PREVIEW_WIDTH, 'height' => $PREVIEW_HEIGHT), BX_RESIZE_IMAGE_PROPORTIONAL, true);

					$arElementSort[$ar["ID"]] = $ar;

				}

				foreach ($elementIDFilter as $sort => $itemID) {
					if (isset($arElementSort[$itemID])) {
						$arElementSort[$itemID]["SORT"] = $sort;
						$j++;
					}
				}
				uasort($arElementSort, "sortBySortField");

				$arResult["CATEGORIES"][$i]["ITEMS"] = $arElementSort;
				$arResult["ELEMENTS"] = $arElementSort;
			}
