...
if (!preg_match('/^(asc|desc|nulls)(,asc|,desc|,nulls){0,1}$/i', (string)$arParams["ELEMENT_SORT_ORDER"]))
	$arParams["ELEMENT_SORT_ORDER"] = "asc";
if (empty($arParams["ELEMENT_SORT_FIELD2"]))
	$arParams["ELEMENT_SORT_FIELD2"] = "id";
if (!preg_match('/^(asc|desc|nulls)(,asc|,desc|,nulls){0,1}$/i', (string)$arParams["ELEMENT_SORT_ORDER2"]))
	$arParams["ELEMENT_SORT_ORDER2"] = "desc";



if ($arParams["ELEMENT_SORT_ORDER"] == 'desc' and $APPLICATION->GetCurPage(false) == '/catalog/mezhkomnatnye_dveri/belye/') {
    $arParams["ELEMENT_SORT_ORDER"] = "asc";
}
...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
	...
	# если есть фильтр - мержим
	if(trim($arResult['~UF_FILTER'])) {
		$tmpFilter = preg_split('#(\r)?\n#', trim($arResult['~UF_FILTER']));
		$secFilter = array();
		foreach($tmpFilter as $secFilterStr) {
			$pos = mb_strrpos($secFilterStr, '=');
			$secFilterStr = array(
				mb_substr($secFilterStr, 0, $pos),
				mb_substr($secFilterStr, $pos+1)
			);

			if(preg_match('#^\d+$#', $secFilterStr[1])) {
				$secFilterStr[1] = (int)$secFilterStr[1];
			}

			if(mb_strpos($secFilterStr[1], ',')!==false) {
				$secFilterStr[1] = explode(',', $secFilterStr[1]);
			}

			if(!$secFilterStr[1]) {
				$secFilterStr[1] = 0;
			}
			if(mb_strpos($secFilterStr[0], 'OFFERS|') === 0) {
				$secFilter['OFFERS'][mb_substr($secFilterStr[0], 7)] = $secFilterStr[1];
			}
			else {
				$secFilter[$secFilterStr[0]] = $secFilterStr[1];
			}

		}

		if(isset($secFilter['OFFERS'])) {
			$subQuery = CIBlockElement::SubQuery("PROPERTY_".$arResult['CATALOG']["SKU_PROPERTY_ID"], array_merge(array(
			...
			if(isset($arFilter['=ID'])) {
				$arFilter[] = array(
					'LOGIC' => 'OR',
					array('=ID' => $arFilter['=ID']),
					array('=ID' => $subQuery)
				);
				unset($arFilter['=ID']);
			}
			else {
				$arFilter['=ID'] = $subQuery;
			}
			unset($secFilter['OFFERS']);
		}

		$arFilter = array_merge($arFilter, $secFilter);
	}

	//__(array_merge($arrFilter, $arFilter));
	...
	while($arItem = $rsElements->GetNext())
	{
        ...
        if(!empty($arItem['PREVIEW_PICTURE']))
            CIBlockPriceToolsNewsite::$fileID[$arItem['PREVIEW_PICTURE']]  = $arItem['PREVIEW_PICTURE'];

        if(!empty($arItem['DETAIL_PICTURE']))
            CIBlockPriceToolsNewsite::$fileID[$arItem['DETAIL_PICTURE']]  = $arItem['DETAIL_PICTURE'];
    ...
    foreach ($arResult["ITEMS"] as $offerKeyId => &$offerValId) {
        foreach (['PREVIEW_PICTURE', 'DETAIL_PICTURE'] as $codeName) {
            ...
            $imageData = !empty(CIBlockPriceToolsNewsite::$fileID[$offerValId[$codeName]]) ? CIBlockPriceToolsNewsite::$fileID[$offerValId[$codeName]] : false;
            ...
            $offerValId[$codeName] = $imageData;

            unset($imageData);

        }
        $arElementLink[$offerValId['ID']] = $offerValId;
    }
	...
	foreach($tmpFilter as $secFilterStr) {
		...
		if(preg_match('#^\d+$#', (string)$secFilterStr[1])) {
			$secFilterStr[1] = (int)$secFilterStr[1];
		}
		if(!$secFilterStr[1]) {
			$secFilterStr[1] = 0;
		}
		if( preg_match('#OFFERS.*#ui', (string)$secFilterStr[0]) ){
			$colors = $secFilterStr[1];
		}
	}
	...
	)
	{
		...
		$arOffers = CIBlockPriceToolsNewsite::GetOffersArray(
			$offersFilter,
			$arResult["ELEMENTS"],
			array(
		...
		if (!empty($arOffers))
		{
			...
			foreach ($arElementLink as &$item)
			{
				if( eRU($colors) ){
					...
					# определяем цвет товара у которого есть
					$colors_ = array();
					foreach($itemOffers as $arOffer){
						if( $arOffer['PROPERTIES']['COLOR']['VALUE'] ){
							$colors_[$arOffer['ID']] = $arOffer['PROPERTIES']['COLOR']['VALUE'];
						}
					}
					foreach( $colors as $color ){
						if( in_array($color, $colors_) ){
							$item['OFFER_ID_SELECTED'] = array_search($color, $colors_);
							break;
						}
					}
					# ---->
