...
$arYandexFields = array(
    ...
    'place', 'hall', 'hall_part', 'is_premiere', 'is_kids', 'date','name'
);
...
$formatList = array(
    ...
    'vendor.model' => array(
        'typePrefix', 'vendor', 'vendorCode', 'model', 'name', 'manufacturer','sales_notes', 'manufacturer_warranty', 'country_of_origin',
        'adult','description'
    ),
    'book' => array(
    ...
    function yandex_get_value($arOffer, $param, $PROPERTY, $arProperties, $arUserTypeFormat, $usedProtocol) {
        ...
        if (isset($arProperties[$PROPERTY]) && !empty($arProperties[$PROPERTY])) {
            ...
            if (!isset($arOffer['PROPERTIES'][$PROPERTY_CODE]) && !isset($arOffer['PROPERTIES'][$PROPERTY]))
                return $strProperty;
            $arProperty = (
                    isset($arOffer['PROPERTIES'][$PROPERTY_CODE]) ? $arOffer['PROPERTIES'][$PROPERTY_CODE] : $arOffer['PROPERTIES'][$PROPERTY]
                    );
                    ...
                    if ($arProperty['MULTIPLE'] == 'Y') {
                        if (!empty($arProperty['~VALUE'])) {
                            ...
                            foreach ($arProperty["~VALUE"] as $oneValue) {
                                ...
                                if (
                                        ($isArray && !empty($oneValue)) || (!$isArray && $oneValue != '')
                                ) {
                    ...
                    if (!empty($arProperty['VALUE'])) {
                        ...
                        if (!empty($arCheckValue)) {
                            $dbRes = CIBlockElement::GetList(
                                            array(), array('IBLOCK_ID' => $arProperty['LINK_IBLOCK_ID'], 'ID' => $arCheckValue), false, false, array('NAME')
                            );
        ...
        if ($PROPERTY_CODE == "SIZE") {
            $_SESSION['name_suffix']['size'] = yandex_text2xml($value, true);
        }
        if ($PROPERTY_CODE == "COLOR") {
            $_SESSION['name_suffix']['color'] = yandex_text2xml($value, true);
        }
        if ($PROPERTY_CODE == "PRODUCT_TYPE") {
            $_SESSION['name_suffix']['type'] = yandex_text2xml($value, true);
        }
        if ($PROPERTY_CODE == "MANUFACTURER") {
            $_SESSION['name_suffix']['manufacturer'] = yandex_text2xml($value, true);
        }
        return $strProperty;
    }
...
/* elseif (!CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, 'iblock_admin_display'))
  ...
  $arRunErrors[] = str_replace('#IBLOCK_ID#',$IBLOCK_ID,GetMessage('CET_ERROR_IBLOCK_PERM'));
    ...
    $rsProps = CIBlockProperty::GetList(
                    array('SORT' => 'ASC', 'NAME' => 'ASC'), array('IBLOCK_ID' => $IBLOCK_ID, 'ACTIVE' => 'Y', 'CHECK_PERMISSIONS' => 'N')
    );
...
if (empty($arCatalog)) {
    ...
    if (!empty($arOffers['IBLOCK_ID'])) {
        ...
        if (($arOfferIBlock = $rsOfferIBlocks->Fetch())) {
            $boolOffers = true;
            $rsProps = CIBlockProperty::GetList(
                            array('SORT' => 'ASC', 'NAME' => 'ASC'), array('IBLOCK_ID' => $intOfferIBlockID, 'ACTIVE' => 'Y', 'CHECK_PERMISSIONS' => 'N')
            );
            while ($arProp = $rsProps->Fetch()) {
                $arProp['ID'] = (int) $arProp['ID'];
...
if (empty($arRunErrors)) {
    ...
    $arCurrencyAllowed = array($RUR, 'USD', 'EUR', 'UAH', 'RUB', 'BYN', 'KZT');
    ...
    if ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_CATALOG || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_OFFERS) {
        ...
        while ($obElement = $res->GetNextElement()) {
			...
			$strTmpOff .= "<vendorCode>BelwoodDoors</vendorCode>\n";
			//$strTmpOff .= "<vendor>BelwoodDoors</vendor>\n";
                    ...
                    case 'description':
                        $strTmpOff .= "<description>" .
                                yandex_text2xml(TruncateText(
                                                ...
                                                strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["~PREVIEW_TEXT"])) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arAcc["~PREVIEW_TEXT"])), 2900), true) .
                                "</description>\n";
                        break;
                    case 'param':
                        if ($parametricFieldsExist) {
                    ...
                    case 'title':
                        if (!$fieldsExist || !isset($fields[$key])) {
                            if (
                                ...
                                $strOfferYandex .= "<" . $key . ">" . yandex_text2xml($arAcc["~NAME"], true) . "</" . $key . ">\n";
                        }
                        else {
                            $strValue = '';
            ...
            if ($arItem['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_SKU) {
                ...
                if (YANDEX_SKU_EXPORT_MIN_PRICE == $arSKUExport['SKU_EXPORT_COND']) {
                    ...
                    if (!empty($arCurrentOffer) && !empty($arCurrentPrice)) {
						...
						//$strOfferYandex .= "<vendorCode>BelwoodDoors</vendorCode>\n";
                        $strOfferYandex .= "<vendor>BelwoodDoors</vendor>\n";
                        if ($minPrice < $fullPrice)
                                ...
                                case 'description':
                                    ...
                                    if (strlen($arOfferItem['~PREVIEW_TEXT']) <= 0) {
                                        ...
                                        $strOfferYandex .= yandex_text2xml(TruncateText(
                                                        ...
                                                        strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arOfferItem["~PREVIEW_TEXT"])) : $arOfferItem["~PREVIEW_TEXT"]), 2900), true);
                                    }
                                    $strOfferYandex .= "</description>\n";
                                    break;
                                case 'param':
                    ...
                    while ($obOfferItem = $rsOfferItems->GetNextElement()) {
						...
						//task 1215
						//$arOfferItem['YANDEX_AVAILABLE'] = ($arOfferItem['CATALOG_AVAILABLE'] == 'Y' ? 'true' : 'false');
                        $arOfferItem['YANDEX_AVAILABLE'] = ($arOfferItem['CATALOG_QUANTITY'] > 0 ? 'true' : 'false');
                        ...
                        if (!$disableReferers)
                            $referer = (strpos($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;') . 'r1=<?=$strReferer1; ?>&amp;r2=<?=$strReferer2; ?>';
                        //$strOfferYandex .= "<url>" . $usedProtocol . $ar_iblock['SERVER_NAME'] . htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]) . "#offer" . $arOfferItem['ID'] . $referer . "</url>\n";
                        $strOfferYandex .= "<url>" . $usedProtocol . $ar_iblock['SERVER_NAME'] . htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"]). "#offer" . $arOfferItem['ID'] . $referer . "</url>\n";

						//$strOfferYandex .= "<price from='true'>" . number_format($minPrice, 2, '.', '') . "</price>\n";
						//task 1215
                        $strOfferYandex .= "<price>" . number_format($minPrice, 2, '.', '') . "</price>\n";
						//$strOfferYandex .= "<vendorCode>BelwoodDoors</vendorCode>\n";
                        $strOfferYandex .= "<vendor>BelwoodDoors</vendor>\n";
                        if ($minPrice < $fullPrice) {
                            $strOfferYandex .= "<oldprice>" . $fullPrice . "</oldprice>\n";
                        }
                        $strOfferYandex .= "<sales_notes>" . yandex_text2xml('Необходима предоплата.', true) . "</sales_notes>\n";
                        $strOfferYandex .= "<currencyId>RUB</currencyId>\n";
                            ...
                            switch ($key) {
                                case 'description':
                                    ...
                                    if (strlen($arOfferItem['~PREVIEW_TEXT']) <= 0) {
                                        ...
                                        $strOfferYandex .= yandex_text2xml(TruncateText(
                                                        ($arOfferItem["PREVIEW_TEXT_TYPE"] == "html" ?
                                                        strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arOfferItem["~PREVIEW_TEXT"])) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arOfferItem["~PREVIEW_TEXT"])), 2900), true);
                                    }
                                    $strOfferYandex .= "</description>\n";
                                    break;
                                case 'param':
                                ...
                                case 'name':
                                    if ($yandexFormat == 'artist.title')
                                        continue;
                                    if ($yandexFormat == 'vendor.model') {
                                       //    echo "<pre>"; print_r($_SESSION['name_suffix']); exit;
                                //        $strOfferYandex .="<vendorCode>BelwoodDoors</vendorCode>";
                                        $strOfferYandex .= "<name>".$APPLICATION->ConvertCharset('Межкомнатная дверь', LANG_CHARSET, 'windows-1251'). ' ' . yandex_text2xml($ar_res["~NAME"], true) . ' ' . $_SESSION['name_suffix']['color'] . ' - ' . $_SESSION['name_suffix']['size'] . "</name>\n";
                                    } else {
                                        $strOfferYandex .= "<name>" . yandex_text2xml($arOfferItem["~NAME"], true) . "</name>\n";
                                    }
                                    break;
                                ...
                                case 'title':
                                    if (!$fieldsExist || !isset($fields[$key])) {
                                        ...
                                        if (
                                            ...
                                            $intElementID = $arOfferItem["~ID"]; // ID предложения
                                            ...
                                            if ($mxResult) {
                                                ...
                                                if ($ar_res = $res->GetNext()) {
                                                    $strOfferYandex .= "<" . $key . ">" . yandex_text2xml($ar_res['NAME'], true) . "</" . $key . ">\n";
                                                }
                                            }
                                        }
                    ...
                    switch ($key) {
                        ...
                        case 'description':
                            $strValue = "<description>" .
                                    yandex_text2xml(TruncateText(
                                                    ($arItem["PREVIEW_TEXT_TYPE"] == "html" ?
                                                    strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arItem["~PREVIEW_TEXT"])) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arItem["~PREVIEW_TEXT"])), 2900), true) .
                                    "</description>\n";
                            break;
                        case 'param':
                            if ($parametricFieldsExist) {
